...
(defn cube-intersection
  ...
  (let [intersectoid
        (into {}
              (for [dim [:x :y :z]]
                [dim [(apply max (map #(first (dim %)) [a b]))
                      (apply min (map #(second (dim %)) [a b]))]]))]
...
(defn process-overlaps
  [processed new-step]
  (apply conj (if (= "on" (:state new-step))
                (conj processed new-step)
                processed)
         (concat (->> processed
